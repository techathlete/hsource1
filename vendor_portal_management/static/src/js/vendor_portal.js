odoo.define('vendor_portal_management.vendor_portal', function(require) {
    ...
    $('document').ready(function () {
         // Product Update
        $('.edit_product_vp').on('click', function (ev) {
        ...
        $('.toggle_active_product_vp').on('click', function (ev) {
            var productID = parseInt(ev.currentTarget.id);
       ...
       // Import Pharmaceuticals
        $('.import_pharma_vp').on('click', function (ev) {
            rpc.query({
                model: 'vendor.product',
                method: 'return_import_configs',
                context: session.user_context,
            }).then(function(values) {
                var select = new VendorProductImportPharma(this, {}, values);
                var def = $.Deferred();
                select.on('save', this, function (root) {
                    def.resolve(root);
                });
       ...
       // Import Medical
        $('.import_medical_vp').on('click', function (ev) {
            rpc.query({
                model: 'vendor.product',
                method: 'return_import_configs',
                context: session.user_context,
            }).then(function(values) {
                var select = new VendorProductImportMedical(this, {}, values);
                var def = $.Deferred();
                select.on('save', this, function (root) {
                    def.resolve(root);
                });
       ...
       // Import Capital
        $('.import_capital_vp').on('click', function (ev) {
            rpc.query({
                model: 'vendor.product',
                method: 'return_import_configs',
                context: session.user_context,
            }).then(function(values) {
                var select = new VendorProductImportCapital(this, {}, values);
                var def = $.Deferred();
                select.on('save', this, function (root) {
                    def.resolve(root);
                });
        ...
        $('.import_stocks_vp').on('click', function (ev) {
            rpc.query({
                model: 'vendor.product',
                method: 'return_import_configs',
                context: session.user_context,
        ...
        // Edit price
        $('.edit_price').on('click', function (ev) {
        ...
        // Archive price
        $('.remove_price').on('click', function (ev) {
            var priceID = parseInt(ev.currentTarget.id);
            rpc.query({
                model: 'product.supplierinfo',
                method: 'get_this_price_values',
        ...
        // Edit stock
        $('.edit_quant').on('click', function (ev) {
            var priceID = parseInt(ev.currentTarget.id);
            rpc.query({
                model: 'vendor.quant',
        ...
        // Register new stock
        $('.add_quant').on('click', function (ev) {
            var productID = parseInt(ev.currentTarget.id);
            rpc.query({
                model: 'vendor.quant',
        ...
        // Archive stock
        $('.remove_quant').on('click', function (ev) {
         ...
         // Location Update
        $('.edit_location_vp').on('click', function (ev) {
        ...
        // Location Create
        $('.create_location_vp').on('click', function (ev) {
        ...
        $('.toggle_active_location_vp').on('click', function (ev) {
            var locationID = parseInt(ev.currentTarget.id);
    ...
    // PHARMA //
    var VendorProductImportPharma = Dialog.extend({
        template: 'vendor_portal_management.vendor_product_import_pharma',
        ...
        init: function (parent, options, values) {
            // Re-write to initiate dialog form with default values
            this.vendor_product_help = values.vendor_pharma_help;
            this.template_table = values.pharma_table;
            this.cur_help = values.cur_help;
            this._super(parent, _.extend({}, {
                title: _t("Import Pharmaceuticals"),
                buttons: [
        ...
        save: function (ev) {
            ...
            this.get_field_values().then(function (values) {
                rpc.query({
                    model: 'vendor.product',
                    method: 'import_product_pharma_portal',
                    args: [values],
                    context: session.user_context,
    ...
    // MEDICAL //
    var VendorProductImportMedical = Dialog.extend({
        template: 'vendor_portal_management.vendor_product_import_medical',
        events: _.extend({}, Dialog.prototype.events, {
            "change #import_chosen_lines": "_changeVisibility",
            "change #archive_products": "_changeVisibility",
        }),
        init: function (parent, options, values) {
            // Re-write to initiate dialog form with default values
            this.vendor_medical_help = values.vendor_medical_help;
            this.template_table = values.medical_table;
            this.cur_help = values.cur_help;
            this._super(parent, _.extend({}, {
                title: _t("Import Medical Products"),
                buttons: [
                    {text: options.save_text || _t("Import"), classes: "btn-primary o_save_button", click: this.save},
                    {text: _t("Cancel"), close: true}
                ]
        ...
        save: function (ev) {
            ...
            this.get_field_values().then(function (values) {
                rpc.query({
                    model: 'vendor.product',
                    method: 'import_product_medical_portal',
                    args: [values],
                    context: session.user_context,
                }).then(function(res_values) {
                    self.destroyAction = "save";
    ...
    // CAPITAL //
    var VendorProductImportCapital = Dialog.extend({
        template: 'vendor_portal_management.vendor_product_import_capital',
        events: _.extend({}, Dialog.prototype.events, {
            "change #import_chosen_lines": "_changeVisibility",
            "change #archive_products": "_changeVisibility",
        }),
        init: function (parent, options, values) {
            // Re-write to initiate dialog form with default values
            this.vendor_medical_help = values.vendor_capital_help;
            this.template_table = values.capital_table;
            this.cur_help = values.cur_help;
            this._super(parent, _.extend({}, {
                title: _t("Import Capital Products"),
                buttons: [
                    {text: options.save_text || _t("Import"), classes: "btn-primary o_save_button", click: this.save},
                    {text: _t("Cancel"), close: true}
                ]
        ...
        save: function (ev) {
            ...
            this.get_field_values().then(function (values) {
                rpc.query({
                    model: 'vendor.product',
                    method: 'import_product_capital_portal',
                    args: [values],
                    context: session.user_context,
                }).then(function(res_values) {
                    self.destroyAction = "save";
