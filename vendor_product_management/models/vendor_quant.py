...
class vendor_quant(models.Model):
        ...
        for quant in self:
            ...
            if quant.vendor_product_id.product_id and quant.supplier_product_uom_id:
                ...
                try:
                    ...
                    product_quantity = quant.supplier_quantity
                    uom_error = _("Vendor UoM doesn't match the Linked Product UoM")
            elif not quant.vendor_product_id.product_id:
                uom_error = _("There is no Linked Product")
            else:
                uom_error = _("There is no Vendor UoM")
    ...
    supplier_product_uom_id = fields.Many2one(
        'uom.uom',
        'Vendor UoM',
        help="if not set, the default product purchase UoM is used",
    )
