...
class vendor_import_result(models.TransientModel):
    ...
    def action_open_results(self):
        ...
        if self.model == "vendor.product.import":
            action = self.env.ref("vendor_product_management.vendor_product_action").read()[0]
            action["context"] = {"search_default_id": record_ids}
        else:
            action = self.env.ref("vendor_product_management.vendor_quant_action").read()[0]
            action["context"] = {"search_default_id": record_ids}
        return action
