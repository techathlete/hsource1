...
class vendor_medical_import(models.TransientModel):
    """
    The model to reflect low sold product variants
    """
    _name = "vendor.medical.import"
    ...
    def _default_template_table_id(self):
        ...
        """
        attach_id = int(self.env['ir.config_parameter'].sudo().get_param('vendor_medical_import_id', 0))
    ...
    def _default_help_text(self):
        ...
        """
        return self.env['ir.config_parameter'].sudo().get_param('vendor_medical_help', "")
    ...
    def _process_import(self):
        ...
        fields = [
                  ...
                  "sell_quantity","uom","total_box_in_case","total_each_in_box",
        ...
        for val in values:
            ...
            sell_quantity = val[8]
            uom = val[9].upper()
            total_box_in_case = val[10]
            total_each_in_box = val[11]
            price, price_error = self._parse_float(val[12])
            currency, currency_error = self._parse_currency(val[13])
            min_qty, min_qty_error = self._parse_float(val[14])
            date_start, date_start_error = self._parse_date(val[15])
            date_end, date_end_error = self._parse_date(val[16])
            location = val[17]
            delay = val[18]
            cost_center = val[19]
            if not product_code or not product_name:
                errors.append(
                    _(u"There is no UDC GTIN Code or Product Description on line {} after the header. Line is skipped;".format(line_num))
                )
                continue
            if not location:
                errors.append(
