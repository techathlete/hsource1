...
class vendor_import_wizard(models.TransientModel):
    ...
    table_to_import = fields.Binary(
        string="File",
    )
    import_chosen_lines = fields.Boolean(string="Skip Header Lines",default=True,readonly=True)
    lines_start = fields.Integer(string="Real header at Row", default=9)
    lines_end = fields.Integer(string="End", default=1002)

    _sql_constraints = [
        (
            'check_positive_start',
            'check (lines_start>=2)',
             _('Starting line should be at least 2, the first line is always a header!')
        ),
        (
            'check_range',
            'check (lines_end >= lines_start)',
    ...
    def action_import(self):
        ...
        action = self.env.ref("vendor_product_management.vendor_import_result_action").read()[0]
    ...
    def _parse_currency(self, value):
        ...
        error = res = False
        if value:
            currency_id = self.env["res.currency"]._search([("name", "=", value)], limit=1)
            if currency_id:
                res = currency_id[0]
            else:
