...
class vendor_pharma_import(models.TransientModel):
    """
    The model to reflect low sold product variants
    """
    _name = "vendor.pharma.import"
    ...
    def _default_template_table_id(self):
        ...
        """
        attach_id = int(self.env['ir.config_parameter'].sudo().get_param('vendor_pharma_import_id', 0))
    ...
    def _default_help_text(self):
        ...
        """
        return self.env['ir.config_parameter'].sudo().get_param('vendor_pharma_help', "")
    ...
    def _process_import(self):
        ...
        # 2
        fields = [
                  "id","product_code","labeler","product_name","description","lot_number","expiration_date","strength",
                  "dosage_form","package_description","sell_quantity","uom","total_box_in_case","total_each_in_box",
                  "price","currency","min_qty","date_start","date_end","location","delay", "cost_center"
                 ]
        ...
        for val in values:
            ...
            description = val[4]
            lot_number = val[5]
            expiration_date = val[6]
            strength = val[7]
            dosage_form = val[8]
            package_description = val[9]
            sell_quantity = val[10]
            uom = val[11].upper()
            total_box_in_case = val[12]
            total_each_in_box = val[13]
            price, price_error = self._parse_float(val[14])
            currency, currency_error = self._parse_currency(val[15])
            min_qty, min_qty_error = self._parse_float(val[16])
            date_start, date_start_error = self._parse_date(val[17])
            date_end, date_end_error = self._parse_date(val[18])
            location = val[19]
            delay = val[20]
            cost_center = val[21]
            if not product_code or not product_name:
                errors.append(
                    _(u"There is no NDC Package Code or Product Description defined on line {} after the header. Line is skipped;".format(line_num))
                )
                continue
            if not location:
                errors.append(
                    _(u"There is no location defined on line {} after the header. The line is skipped;".format(line_num))
                )
                continue
            ...
            pr_values = {
                ...
                "labeler": labeler,
                "description": description, "lot_number": lot_number, "expiration_date": expiration_date, "strength": strength,
                "dosage_form": dosage_form, "package_description": package_description, "sell_quantity": sell_quantity,
