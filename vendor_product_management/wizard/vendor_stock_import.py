...
class vendor_stocks_import(models.TransientModel):
    ...
    archive_stocks = fields.Boolean(
        string="Replace existing Inventory",
    )
    ...
    def _process_import(self):
        ...
        for val in values:
            ...
            if uom_name:
                uom_ids = self.env["uom.uom"]._search([("name", "=", uom_name)], limit=1)
                if uom_ids:
                    supplier_product_uom_id = uom_ids[0]
                else:
                    errors.append(
                        _(u"Unit of measure name {} of the line {} is not found".format(
                            uom_name,
                            line_num,
                        ))
                    )
